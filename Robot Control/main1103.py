...
from def_linecomeback_green import *
...
#=========================================================
    ...
    lower_black = (5, 0, 0) 
    upper_black = (48, 255, 255)
    img_mask_black = cv2.inRange(img_hsv, lower_black, upper_black) 
#_================------------------------====================
    cntNotyellow=cv2.countNonZero(img_mask_black)
    return cntNotyellow
...
def cnt1_1(cap,serial_port,alphabet_color,floor_color,arrow_direction):
    ...
    if arrow_direction==right_turn:
        TX_data_py2(serial_port,30)#neck moving
    elif arrow_direction==left_turn:
        TX_data_py2(serial_port,28)
    div_gb,div_inout_inblack_object,time_break=contour_main(cap,serial_port,alphabet_color,floor_color,arrow_direction)
    #if(end_contour1==end):
    #    break
    #while True:
    time.sleep(2.5)
    ...
    time.sleep(1.5)
    '''
    if time_break==time_break_none:
        if div_gb==div_danger and div_inout_inblack_object==div_in:##edit1113##
            time.sleep(1)
            TX_data_py2(serial_port,67)
            time.sleep(2)
    ...
    #=====================================edit1113=================
        elif div_gb==div_safe:
            
            TX_data_py2(serial_port,58)     #23 neck
            for i in range(7):
                imgsdf=cap.read()
                time.sleep(0.1)
            time.sleep(1.0)
            end_linedetectb1=def_linecomback_main_green(cap,serial_port,0)
            time.sleep(1.5)
...
while True:
    ...
    if t!=50:   #corner none
        ...
        if cnt1==0 and t==69:###1107editt==67:
            t=70##1107edit#t=65
        if t==1 or t==3 or t==59 or t==71:
            if t!=711 and t!=71:
                TX_data_py2(serial_port,t)
            elif t==71 :###edit1113###and (cnt1==1 or cnt1==3 or cnt1==5):
                time.sleep(2)
    ...
    if t==50:   #corner
        ...
        if cnt1==6:##edit1113cnt1==4:test//#cnt==6: #when go out
            time.sleep(2)
            TX_data_py2(serial_port,67)
            time.sleep(1)
            for i in range(5):
